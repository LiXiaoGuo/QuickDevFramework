...
object CacheManager {
    //默认的缓存地址
    var CACHE_PATH = FileManager.getExternalStorageRoot().toString() + File.separator + BaseApplication.getApplicationName() + File.separator + "cache"
    val dlc:DiskLruCache by lazy { DiskLruCache.open(FileUtil.getFile(CACHE_PATH),1,1, 10*1024*1024) }
